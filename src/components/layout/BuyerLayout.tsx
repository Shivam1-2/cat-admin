      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="relative flex flex-col w-full max-w-xs h-full bg-white">
            ...
            <BuyerSidebar isMobile onClose={() => setSidebarOpen(false)} />
          </div>
        </div>
      )}
      
      <div className="flex flex-col flex-1 w-0 overflow-hidden">
        <BuyerHeader openSidebar={() => setSidebarOpen(true)} />
        
        <main className="flex-1 relative overflow-y-auto focus:outline-none bg-emerald-50 p-4 md:p-6">
          {children}
        </main>
      </div>
    </div>
